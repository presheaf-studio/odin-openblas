...
) -> BandedMatrix(T) where is_float(T) ||
    is_complex(T) {
...
band_to_triband :: proc(
    bm: ^BandedMatrix($T),
    diag: Diag = .NonUnit,
) -> TriBand(T) where is_float(T) ||
    is_complex(T) {
