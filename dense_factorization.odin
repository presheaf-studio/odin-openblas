...
dns_qr_pivoted_complex :: proc(
	...
	when Cmplx == complex64 {
		lapack.cgeqpf_(&m, &n, raw_data(A.data), &lda, raw_data(jpvt), raw_data(tau), raw_data(work), raw_data(rwork), &info)
	} else when Cmplx == complex128 {
		lapack.zgeqpf_(&m, &n, raw_data(A.data), &lda, raw_data(jpvt), raw_data(tau), raw_data(work), raw_data(rwork), &info)
	}
