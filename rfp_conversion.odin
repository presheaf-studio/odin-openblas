...
packed_symmetric_to_rfp :: proc(
    source: ^PackedSymmetric($T),
    trans_state: TransposeState,
    allocator := context.allocator,
) -> (
    rfp: RFP(T),
...
packed_hermitian_to_rfp :: proc(
    source: ^PackedHermitian($T),
    trans_state: TransposeState,
    allocator := context.allocator,
) -> (
    rfp: RFP(T),
...
rfp_to_packed_symmetric :: proc(
    source: ^RFP($T),
    allocator := context.allocator,
) -> (
    ps: PackedSymmetric(T),
    info: Info,
    ok: bool,
) where is_float(T) ||
    is_complex(T) {
...
convert_rfp_format :: proc(
    source: ^RFP($T),
    new_trans_state: TransposeState,
    allocator := context.allocator,
) -> RFP(T) where is_float(T) ||
    is_complex(T) {
...
prepare_rfp_for_lapack :: proc(
    rm: ^RFP($T),
    required_trans: TransposeState,
    required_uplo: UpLo,
    allocator := context.allocator,
) -> (
    result: RFP(T),
