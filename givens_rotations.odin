...
dns_scale :: proc(A: ^Matrix($Real_Or_Cmplx), cfrom, cto: $Real, scaling_type: MatrixScalingType = .General, kl: int = 0, ku: int = 0) -> (info: Info, ok: bool) where (Real_Or_Cmplx == Real) || (Real_Or_Cmplx == complex64 && Real == f32) || (Real_Or_Cmplx == complex128 && Real == f64) {
	// Validate matrix
...
apply_givens_sequence_real :: proc(A: ^Matrix($T), c, s: []T, side: ReflectorSide = .Left, pivot: GivensPivot = .Variable, direction: GivensDirection = .Forward) where is_float(T) {
...
apply_givens_sequence_complex :: proc(A: ^Matrix($Cmplx), c, s: []$Real, side: ReflectorSide = .Left, pivot: GivensPivot = .Variable, direction: GivensDirection = .Forward) where (Cmplx == complex64 && Real == f32) || (Cmplx == complex128 && Real == f64) {
	assert(len(c) == len(s), "Cosine and sine arrays must have same length")
...
dns_scale_safe :: proc(A: ^Matrix($T), scaling_type := MatrixScalingType.General, kl := 0, ku := 0) -> (success: bool, scale_factor: T) where is_float(T) || is_complex(T) {
