...
solve_apply_householder_reflector :: proc(side: ReflectorSide, m: int, n: int, V: ^Vector($T), tau: T, C: ^Matrix(T), work: []T) where is_float(T) || is_complex(T) {
	...
	when T == f32 {
		...
		lapack.clarf_(&side_c, &m_int, &n_int, &V.data[V.offset], &incv, &tau_val, raw_data(C.data), &ldc, raw_data(work))
	} else when T == complex128 {
		lapack.zlarf_(&side_c, &m_int, &n_int, &V.data[V.offset], &incv, &tau_val, raw_data(C.data), &ldc, raw_data(work))
	}
}
...
query_workspace_block_householder_reflector :: proc($T: typeid, m: int, n: int, k: int, side: ReflectorSide) -> (work_size: int) where is_float(T) || is_complex(T) {
...
solve_apply_block_householder_reflector :: proc(
	...
	when T == f32 {
		...
		lapack.clarfb_(&side_c, &trans_c, &direct_c, &storev_c, &m_int, &n_int, &k_int, raw_data(V.data), &ldv, raw_data(T_matrix.data), &ldt, raw_data(C.data), &ldc, raw_data(work), &ldwork_int)
	} else when T == complex128 {
		lapack.zlarfb_(&side_c, &trans_c, &direct_c, &storev_c, &m_int, &n_int, &k_int, raw_data(V.data), &ldv, raw_data(T_matrix.data), &ldt, raw_data(C.data), &ldc, raw_data(work), &ldwork_int)
	}
}
...
form_triangular_block_householder :: proc(
	...
	when T == f32 {
		...
		lapack.clarft_(&direct_c, &storev_c, &n_int, &k_int, raw_data(V.data), &ldv, raw_data(tau), raw_data(T_matrix.data), &ldt, 1, 1)
	} else when T == complex128 {
		lapack.zlarft_(&direct_c, &storev_c, &n_int, &k_int, raw_data(V.data), &ldv, raw_data(tau), raw_data(T_matrix.data), &ldt, 1, 1)
	}
}
