...
one_norm_triangular :: proc(A: Triangular($T), res_out: ^$R, uplo: MatrixRegion = .Upper, diag: DiagonalType = .NonUnit) where (is_float(T) && R == T) || (is_complex(T) && ((T == complex64 && R == f32) || (T == complex128 && R == f64))) {
	max_col_sum := R(0)
...
infinity_norm_triangular :: proc(A: Triangular($T), res_out: ^$R, uplo: MatrixRegion = .Upper, diag: DiagonalType = .NonUnit) where (is_float(T) && R == T) || (is_complex(T) && ((T == complex64 && R == f32) || (T == complex128 && R == f64))) {
	max_row_sum := R(0)
...
max_norm_triangular :: proc(A: Triangular($T), res_out: ^$R, uplo: MatrixRegion = .Upper, diag: DiagonalType = .NonUnit) where (is_float(T) && R == T) || (is_complex(T) && ((T == complex64 && R == f32) || (T == complex128 && R == f64))) {
	max_val := R(0)
...
norm_triangular :: proc(A: Triangular($T), norm_type: MatrixNorm, res_out: ^$R, uplo: MatrixRegion = .Upper, diag: DiagonalType = .NonUnit) where (is_float(T) && R == T) || (is_complex(T) && ((T == complex64 && R == f32) || (T == complex128 && R == f64))) {
	switch norm_type {
	case .OneNorm:
...
tri_condition_with_actual_real :: proc(A: Triangular($T), work: []T, iwork: []Blas_Int, uplo: MatrixRegion = .Upper, diag: DiagonalType = .NonUnit, norm_type: MatrixNorm = .OneNorm, allocator := context.allocator) -> (rcond: T, cond: T, ok: bool) where is_float(T) {
...
query_workspace_condition_triangular :: proc(A: Triangular($T)) -> (work_size: int, iwork_size: int, rwork_size: int) where is_float(T) || is_complex(T) {
	n := A.n
	when is_float(T) {
